...
# %%--- Initialize the object
    def __init__(self, path, task, repeat, load_data_from_path = True):
        ...
        # without svc:
        # regression_default_param = {
        # 'model_names': ['KNN', 'Ridge Linear Regression', 'Random Forest' , 'Neural Network', 'Gradient Boosting', 'Ada Boosting'], # a list of name for each model.
        # 'model_lists': [KNeighborsRegressor(), Ridge(), RandomForestRegressor(n_estimators=100, verbose =0, n_jobs=-1), MLPRegressor(((100, 100)),alpha=0.001, activation = 'relu',verbose=0,learning_rate='adaptive'), GradientBoostingRegressor(verbose=0,loss='ls',max_depth=10), AdaBoostRegressor(base_estimator = DecisionTreeRegressor(), n_estimators=100, loss='linear')],# a list of model improted from sklearn
        # 'gridsearchlist': [False, False, False, False, False, False], # each element in this list corspond to a particular model, if True, then we will do grid search while training the model, if False, we will not do Gridsearch for this model.
        # 'param_list': [{'n_neighbors':range(1, 30)}, {'alpha': [0.01, 0.1, 1, 10]}, {'n_estimators': [200, 100, 1000, 500, 2000], 'verbose':[0], 'n_jobs':[-1]}, {'hidden_layer_sizes':((100, 300, 300, 100), (100, 300, 500, 300, 100), (200, 600, 600, 200), (200, 600, 900, 600, 200), (100, 300, 500, 700, 500, 300, 100)), 'alpha': [0.001], 'learning_rate':['adaptive']}, {'n_estimators':[200, 100]}]# a list of key parameters correspond to the models in the model_lists if we are going to do grid searching
        # }
        # only neural network.
        # regression_default_param = {
        ...
        # 'model_lists': [KNeighborsClassifier(n_neighbors = 5, weights='distance',n_jobs=-1), SVC(), DecisionTreeClassifier(), RandomForestClassifier(n_estimators=100, verbose =0,n_jobs=-1), GradientBoostingClassifier(verbose=0,loss='deviance'), AdaBoostClassifier(base_estimator = DecisionTreeClassifier(), n_estimators=10), GaussianNB(), MLPClassifier((100,100),alpha=0.001, activation = 'relu',verbose=1,learning_rate='adaptive')],# a list of model improted from sklearn
        ...
        'model_lists': [KNeighborsClassifier(n_neighbors = 5, weights='distance',n_jobs=-1), DecisionTreeClassifier(), RandomForestClassifier(n_estimators=100, verbose =0,n_jobs=-1), GradientBoostingClassifier(verbose=0,loss='deviance'), AdaBoostClassifier(base_estimator = DecisionTreeClassifier(), n_estimators=10), GaussianNB(), MLPClassifier((100, 300, 500, 300, 100),alpha=0.001, activation = 'relu',verbose=0,learning_rate='adaptive')],# a list of model improted from sklearn
        ...
        # classification_default_param = {
        # 'model_names': ['Naive Bayes', 'Neural Network'], # a list of name for each model.
        # 'model_lists': [GaussianNB(), MLPClassifier(activation='relu', alpha=0.001, batch_size='auto', beta_1=0.9, beta_2=0.999, early_stopping=False, epsilon=1e-08, hidden_layer_sizes=(100, 100), learning_rate='adaptive', learning_rate_init=0.001, max_iter=200, momentum=0.9, n_iter_no_change=10, nesterovs_momentum=True, power_t=0.5, random_state=None, shuffle=True, solver='adam', tol=0.0001, validation_fraction=0.1, verbose=1, warm_start=False)],# a list of model improted from sklearn
        # 'gridsearchlist': [False, False],
        # 'param_list': [{'var_smoothing':[1e-9, 1e-3]}, {'hidden_layer_sizes':((100, 300, 500, 300, 100), (100, 300, 500, 500, 300, 100), (200, 600, 900, 600, 200))}]# a list of key parameters correspond to the models in the model_lists
        # }
...
# %%--- Regression machine learning tasks.
    ...
    def regression_repeat(self, plot=False, output_y_pred=False, colour_code = True):
        ...
        plt.savefig('R2 score ' + str(self.singletask) + '.png')
        ...
        plt.savefig('MAE score ' + str(self.singletask) + '.png')
        # append the data label for the boxplot
        # for k in range(len(r2_av)):
        #     y = 8.5/(len(r2_av) + 1)*k + 0.5
        #     # x=0.99
...
# %%--- The per-processors before machine learning tasks
    ...
    def pre_processor(self):
        ...
        # drop these columns
        # print(pd.DataFrame(self.data).columns.tolist())
        # print(self.data)
        ...
        for model in model_lists:
            ...
            if plotall == True:
                for k in range(np.shape(y_test)[1]):
                    ...
                    print('transparency of scattering plot is ' + str(alpha))
                    plt.scatter(y_test[:, k], y_pred_ordered[:, k], label='$R^2$=' + str(round(np.max(r2list[k]), 3)) + ' MAE=' + str(round(mae_list[k], 3), alpha=alpha))
                    plt.xlabel('real value')
                    plt.ylabel('prediction')
                    plt.title('real vs prediction using model ' + str(model_names[modelcount]) + ' for ' + tasknamelist[k])
                    plt.legend()
